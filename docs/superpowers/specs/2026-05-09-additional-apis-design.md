...
- エンドポイント: ADS REST API `https://ads.atmosphere.copernicus.eu/api/retrieve/v1` を `requests` で直接呼び出す（`cdsapi` パッケージは使用しない、新規依存なし）
- 認証: リクエストヘッダー `Authorization: Bearer <CAMS_ADS_API_KEY>`
- パラメータ: `aod550`（550nm AOD）、当日予報、熊本周辺グリッド点（lat±0.5°, lon±0.5°）
- 失敗・キー未設定時: `None` を返す
...
- 対象: コード内定数 `KNOWN_COMETS` としてハードコードした彗星指定リストを1件ずつ照会する。初期リストは現在観測可能な主要彗星（例: `C/2025 E3`, `C/2023 A3`, `12P/Pons-Brooks`, `1P/Halley` 等）を含む。新彗星への対応はリスト更新で行う。
